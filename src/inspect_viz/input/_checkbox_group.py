...
def checkbox_group(
    ...
    *,
    ...
    Checkbox groups have a `target` which is either a `Param` or `Selection`. In the latter case,
    ...
    Args:
       ...
       target: A `Param` or `Selection` that this radio group should update. For a `Param`, the selected value is set to be the new param value. For a `Selection`, a predicate of the form column IN (values) will be added to the selection.
    ...
    """
    config: dict[str, Any] = {"input": "checkbox_group"}
