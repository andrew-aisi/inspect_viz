...
export class Slider extends Input {
    ...
    constructor(protected readonly options_: SliderOptions) {
        ...
        // add slider
        const { value, width, min, max, step } = options_;
        ...
        this.sliderApi_ = createSlider(this.slider_, {
            range: {
                min: min || (Array.isArray(value) ? value[0] : value ? value : 0),
                max: max || (Array.isArray(value) ? value[1] : value ? value : 0),
            },
            connect: this.options_.select === 'interval' ? true : undefined,
            step: step,
            start: value || 0,
        });
        if (width != undefined) {
    ...
    query(filter: FilterExpr[] = []) {
        ...
        if (!from || !column || (this.options_.min != null && this.options_.max != null)) {
            return null;
        }
    ...
    queryResult(data: any) {
        ...
        const step = this.options_.step || (max - min) / 500;
    ...
    clause(value?: number | [number, number]): SelectionClause {
        ...
        if (select === 'interval' && value !== undefined) {
            ...
            return clausePoint(field, Array.isArray(value) ? value[0] : value, { source: this });
        }
    }
