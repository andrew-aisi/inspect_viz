...
By default, tables will have a height which matches the size of their content and a width which files their container (with a maximum size of 1000px). You can explicitly provide a height and width value in pixels for the table if you'd like the table to be a specific size:
...
You can use `max_width` to constrain the maximum width of the table in pixels. It will still attempt to fill its container, but it's width will not exceed the `max_width`. You can use `max_height` to constrain the maximum height of an `auto` height table, it's height will be then match the content, but not exceed the maximum height.
