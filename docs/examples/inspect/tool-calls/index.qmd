...
The plot visualizes tool usage over a series of turns in a Cybench evaluation. We use a `cell()` mark to visualize tool use over messages in each sample of an evaluation. We note any limit that ended the sample using a `text()` mark on the right side of the frame. See the [Data Preparation](#data-preparation) section below for details on how the data frame for the plot was created.

Here is the plot and an annotated version of the code required to produce it (click on the numbers in the right margin for additional explanation).
