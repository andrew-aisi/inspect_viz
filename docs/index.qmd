...
1. You want to take advantage of its pre-built [Views](views.qmd) that go directly from Inspect log [data frames](https://inspect.aisi.org.uk/dataframe.html) to visualizations.
2. You want to create plots that include direct [links](components-links.qmd) to the Inspect log viewer (try hovering over the points above and then clicking the log viewer link).
3. You want to create _interactive_ visualizations (either filtering as shown above or directly selecting points on one plot to dynamially drive the contents of other plots).
4. You value the more modern appearance and flexible compositions available in [Observable Plot](https://observablehq.com/plot/), the plotting library Inspect Viz is built on.
...
Inspect Viz [Views](views.qmd) are pre-built plots that work with data created by the Inspect log [data frame](https://inspect.aisi.org.uk/dataframe.html) reading functions. For example, the timeline plot above was created with a single call to `scores_timeline()`:
...
The `scores_with_baseline()` view enables you to view an ordered set of eval scores with one or more baselines:
...
See the article on [Views](views.qmd) to learn about all available pre-built views and the various options for customizing their appearance and behavior.
