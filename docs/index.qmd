...
The [`samples_tool_calls()`](view-samples-tool-calls.qmd) view enables you to visualize tool calls by sample:

```{python}
from inspect_viz.view.beta import samples_tool_calls

tools = Data.from_file("cybench_tools.parquet")
samples_tool_calls(tools, legend=legend("color", frame_anchor="bottom", border=False))
```
`
