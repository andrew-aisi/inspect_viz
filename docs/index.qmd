...
Then, read on below for a tutorial on the basics of Inspect Viz. Once you are up to speed on the basics, read the [Concepts](concepts.qmd) article for a more in depth explanation of the library's components and how they fit together.
...
The `x` axis for this plot is not mapped to a column, but rather to a `count()` transform ( [transforms](reference/inspect_viz.transform.qmd) enable you to perform computations on columns for ploatting). The `fill` option gives each species it's own color. We also specify that we don't want a `y_label` (as the species names serve that purpose) and a smaller than normal height.
...
1. Create a `bandwidth` parameter with a default value of 20.
...
3. The athlete select box is both _filtered by_ and targets the `filter` selection, enabling it to both confine itself to the selected team as well as filter what is displayed in the plot.
...
- [Concepts](concepts.qmd) describes the core components of the library and how they fit together.
