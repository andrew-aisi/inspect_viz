...
Here's an Inspect Viz plot that compares benchmarks scores over time for various providers^[This plot was inspired by and includes data from the [Epoch AI](https://epoch.ai/data/ai-benchmarking-dashboard) Benchmarking Hub]:
...
Inspect Viz [Views](views.qmd) are pre-built plots that work with data created by the Inspect log [data frame](https://inspect.aisi.org.uk/dataframe.html) reading functions. For example, the `scores_by_factor()` view enables you to compare scores across models and a boolean factor:
