...
Welcome to Inspect Viz, a data visualisation library for [Inspect AI](https://inspect.aisi.org.uk/). Inspect Viz provides flexible tools for high quality interactive visualisations from Inspect evaluations. You can include these visualisations in any [Jupyter Notebook](https://jupyter.org/) or web publishing system that supports Jupyter widgets (e.g. [Quarto](https://quarto.org)).

You can use Inspect Viz at two different layers of abstraction:

1.  Using [views](views.qmd), which provide pre-built interactive visualisations for Inspect evaluation data.

2.  Using [components](components-plots.qmd) to create your own custom plots using the same lower-level APIs used to build views.

::: callout-note
Currently there are a fairly limited number of [views](views.qmd)—many more views are under development and we encourage you to [submit your own](https://github.com/meridianlabs-ai/inspect_viz/pulls) as pull requests.
:::
...
Then, read on below for a tutorial on the basics of Inspect Viz. Once you are up to speed on the basics, read the [Views](views.qmd) and [Components](components-plots.qmd) articles for a more in depth explanation of the library's features and how they fit together.
...
For example, here we read a set of evaluation logs and pass it to the `evals_bar_plot()` function, which by default shows scores by task name and model along with confidence intervals:

```{python}
from inspect_viz import Data
from inspect_viz.view.beta import evals_bar_plot

# (this data frame was originally read with `evals_df("logs")`)
evals = Data.from_file("evals.parquet")

# plot the scores by model and task
evals_bar_plot(evals)
```

We can optionally add filtering by model and task name:

```{python}
evals_bar_plot(evals, x_filter=True, fx_filter=True)
```

The `evals_table()` function can be used to view evaluation scores in a tabular display:
...
See the [Views](views.qmd) article for additional details on using thse and other pre-built views.
:::
...
plot(
    dot(penguins, x="body_mass", y="flipper_length",  
        stroke="species", symbol="species", channels={ "sex": "sex", "island": "island"}),
    legend="symbol",
    grid=True
)
```

Here we mapped the "species" column to the `stroke` and `symbol` scales of the plot (causing each species to have its own color and symbol). We also add a `legend` to the plot as a key to this mapping.
...
-   [Views](views.qmd) describes the available built in views for Inspect evaluation data.
