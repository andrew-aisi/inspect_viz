...
Inspect Viz makes it's documentation available in an LLM friendly markdown format. Use the llms.txt file for configuring IDEs that support llms.txt documentation. Use the llms-guide.txt and/or llms-full.txt variants for pasting into model context.
...
You can also configure Claude Code to access the documentation using an MCP server (expand the section below for details).
:::

::: {.callout-note appearance="simple" collapse="true"}
##### Claude Code llms.txt

Execute the following from within a terminal to configure Inspect Viz documentation for the current project:
...
Alternatively, add the following configuration for your project to the Claude Code config file (`~/.claude.json`):
...
In addition, the Inspect Viz Python API maps quite closely to the [Observable Plot](https://observablehq.com/plot/) JavaScript API that it is built on top of. Consequently, asking an LLM how to do things in Observable Plot will typically yield actionable advice.
