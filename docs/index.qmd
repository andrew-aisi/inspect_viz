...
Welcome to Inspect Viz, a data visualisation library for [Inspect AI](https://inspect.aisi.org.uk/). Inspect Viz provides flexible tools for high quality interactive visualisations from Inspect evaluations. You can author visualisations in any [Jupyter Notebook](https://jupyter.org/) then include them in documents as static images or or websites as interactive Jupyter Widgets.

Here's an Inspect Viz plot that compares benchmarks scores over time for varioius providers. Use the filters to switch benchmarks and restrict to models from various organization(s). Hover over the points to get additional details on them.
...
vconcat(
    ...
    checkbox_group(evals, column="organization", options=orgs),
...
Why use a visualisation library specific to Inspect? Why not just use Matplotlib or Plotly which you might already be familiar with and which LLMs have experience writing code for? Inspect Viz is indeed not for every scenario, and it does have a learning curve. You should consider Inspect Viz for situations where:

1. You want to create _interactive_ visualizations (either filtering as shown above or directly selecting points on one plot to dynamially drive the contents of other plots).

2. You want to create plots that include direct links to the Inspect log viewer (try hovering over the points above and then clicking the log viewer link).

3. You value the more modern appearance and flexible compositions available in [Observable Plot](https://observablehq.com/plot/), the plotting library Inspect Viz is built on.
...
Then, read on below for a tutorial on the basics of Inspect Viz. Once you are up to speed on the basics, check out the [Examples](examples/index.qmd)  and read the [Components](components-plots.qmd) articles for a more in depth explanation of the library's features and how they fit together.
